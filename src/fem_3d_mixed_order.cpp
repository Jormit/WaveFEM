	...
	for (size_t p = 0; p < 11; p++)
	{
		...
		for (int i = 0; i < 20; i++)
		{
			for (int j = 0; j < 20; j++)
			{
				...
				S(i, j) += w * (mu_inv * curl_funcs.row(i)).dot(curl_funcs.row(j));
				T(i, j) += w * (ep * basis_funcs.row(i)).dot(basis_funcs.row(j));
			}
		}
	}
	...
	for (const auto& e : surface_elems)
	{
		...
		for (int local_dof_i = 0; local_dof_i < 8; local_dof_i++)
		{
			...
			for (size_t local_dof_j = 0; local_dof_j < 8; local_dof_j++)
			{
				...
				A.coeffRef(global_dof_i, global_dof_j) = A.coeff(global_dof_i, global_dof_j) + gamma * B_local(local_dof_i, local_dof_j);
			}
		}
	}
